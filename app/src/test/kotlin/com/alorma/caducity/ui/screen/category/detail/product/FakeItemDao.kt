...
class FakeItemDao(
  ...
  override fun getCategoryItems(categoryId: String): Flow<List<ItemRoomEntity>> {
    return itemsInMemory.map { items ->
      items.filter { it.categoryId == categoryId }
    }
  }

  override fun getProductItems(categoryId: String, productId: String): Flow<List<ItemRoomEntity>> {
    return itemsInMemory.map { items ->
  ...
  override suspend fun getItem(itemId: String): ItemRoomEntity? {
    return itemsInMemory.value.firstOrNull { it.id == itemId }
  }
  ...
  override suspend fun moveItemsToProduct(fromProductId: String, toProductId: String?) {
    itemsInMemory.value = itemsInMemory.value.map { item ->
  ...
  override suspend fun deleteActiveItemsByProduct(productId: String) {
    itemsInMemory.value = itemsInMemory.value.filterNot {
      it.productId == productId && it.consumedDate == null
    }
  }

  override suspend fun deleteConsumedItemsByProduct(categoryId: String, productId: String) {
    itemsInMemory.value = itemsInMemory.value.filterNot {
      it.categoryId == categoryId && it.productId == productId && it.consumedDate != null
    }
  }

  override suspend fun deleteAllItemsByProduct(categoryId: String, productId: String) {
    itemsInMemory.value = itemsInMemory.value.filterNot {
  ...
  override suspend fun getAllItemsSync(): List<ItemRoomEntity> {
    return itemsInMemory.value
  }
