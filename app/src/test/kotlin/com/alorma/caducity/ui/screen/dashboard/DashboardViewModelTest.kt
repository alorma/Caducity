...
class DashboardViewModelTest {
  ...
  private val relativeTimeFormatter: RelativeTimeFormatter = mock {
  ...
  private val appCalendarConfigMapper: AppCalendarConfigMapper = mock {
  ...
  private val calendarPreferencesState = MutableStateFlow(
    CalendarConfigState(firstDayOfWeek = DayOfWeek.MONDAY)
  )
  private val calendarPreferences: com.alorma.caducity.ui.components.calendar.CalendarPreferences = mock {
  ...
  @Test
  fun `state updates to Success with empty categories`() = runTest {
    // Given
    categoriesFlow.value = persistentListOf()
  ...
  @Test
  fun `state updates to Success with categories and items`() = runTest {
    // Given
    val categoryId = UUID.randomUUID().toString()
    val productId = UUID.randomUUID().toString()
    val product = createTestProduct(id = productId, categoryId = categoryId)

    val freshItem = createTestItem(
      productId = productId,
    ...
    val expiredItem = createTestItem(
      productId = productId,
    ...
    val categoryWithItems = createTestCategory(
      id = categoryId,
      name = "Dairy",
      products = listOf(
        CategoryProduct(
  ...
  @Test
  fun `summary counts frozen items separately`() = runTest {
    // Given
    val categoryId = UUID.randomUUID().toString()
  ...
  @Test
  fun `summary excludes consumed items`() = runTest {
    // Given
    val categoryId = UUID.randomUUID().toString()
    val freshItem = createTestItem(status = ItemStatus.Fresh)
  ...
  @Test
  fun `navigate to CreateCategory emits navigation side effect`() = runTest {
    // Given
    val viewModel = createViewModel()
    testDispatcher.scheduler.advanceUntilIdle()
  ...
  @Test
  fun `navigate to CreateCategory tracks action`() = runTest {
    // Given
    val viewModel = createViewModel()
    testDispatcher.scheduler.advanceUntilIdle()
  ...
  @Test
  fun `navigate to Category emits navigation side effect with category ID`() = runTest {
    // Given
    val viewModel = createViewModel()
    val categoryId = UUID.randomUUID().toString()
    testDispatcher.scheduler.advanceUntilIdle()
  ...
  @Test
  fun `navigate to Category tracks action with source`() = runTest {
    // Given
    val viewModel = createViewModel()
    val categoryId = UUID.randomUUID().toString()
    testDispatcher.scheduler.advanceUntilIdle()
  ...
  @Test
  fun `navigate to FilteredItems emits navigation side effect with status`() = runTest {
    // Given
    val viewModel = createViewModel()
    testDispatcher.scheduler.advanceUntilIdle()
  ...
  @Test
  fun `navigate to FilteredItems tracks action with correct status param`() = runTest {
    // Given
    val viewModel = createViewModel()
    testDispatcher.scheduler.advanceUntilIdle()
  ...
  @Test
  fun `navigate to Settings emits navigation side effect`() = runTest {
    // Given
    val viewModel = createViewModel()
    testDispatcher.scheduler.advanceUntilIdle()
  ...
  @Test
  fun `navigate to Settings tracks action`() = runTest {
    // Given
    val viewModel = createViewModel()
    testDispatcher.scheduler.advanceUntilIdle()
