...
class FilteredItemsByStatusViewModelTest {
  ...
  private val getItemsByStatusUseCase: GetItemsByStatusUseCase = mock {
  ...
  private fun createViewModel(status: ItemStatus = ItemStatus.Expired): FilteredItemsByStatusViewModel {
    return FilteredItemsByStatusViewModel(
      status = status,
  ...
  @Test
  fun `state updates to Empty when no items match status`() = runTest {
    // Given
    categoriesFlow.value = persistentListOf()
  ...
  @Test
  fun `state updates to Success with filtered expired items`() = runTest {
    // Given
    val categoryId = UUID.randomUUID().toString()
    val productId = UUID.randomUUID().toString()
    val product = createTestProduct(id = productId, categoryId = categoryId)

    val expiredItem1 = createTestItem(
      productId = productId,
    ...
    val expiredItem2 = createTestItem(
      productId = productId,
    ...
    viewModel.state.test {
      ...
      expectThat(successState.categories[0].products[0].items.size).isEqualTo(2)
    }
  }

  @Test
  fun `state updates to Success with frozen items`() = runTest {
    // Given
    val categoryId = UUID.randomUUID().toString()
    val frozenItem = createTestItem(
  ...
  @Test
  fun `state updates to Success with multiple categories`() = runTest {
    // Given
    ...
    val category1 = createTestCategory(
      id = category1Id,
      name = "Dairy",
      standaloneItems = listOf(expiredItem1),
    )

    val category2 = createTestCategory(
      id = category2Id,
      name = "Produce",
  ...
  @Test
  fun `onProductClick emits bottom sheet side effect`() = runTest {
    // Given
    val viewModel = createViewModel()
    testDispatcher.scheduler.advanceUntilIdle()
  ...
  @Test
  fun `onItemClick emits bottom sheet side effect`() = runTest {
    // Given
    val viewModel = createViewModel()
    testDispatcher.scheduler.advanceUntilIdle()
