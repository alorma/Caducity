...
class AiJaroWinklerMatcherTest {
  ...
  private val cheeseProduct = Product(
    id = "prod-2",
    categoryId = "cat-1",
    name = "Cheese",
    createdAt = Instant.fromEpochSeconds(0),
  )
  private val beefProduct = Product(
    id = "prod-3",
    categoryId = "cat-2",
    name = "Beef",
    createdAt = Instant.fromEpochSeconds(0),
  ...
  private fun matcher(vararg categoryWithItems: CategoryWithItems): AiJaroWinklerMatcher {
    val dataSource = mock<CategoryDataSource> {
  ...
  @Test
  fun `exact match returns Match with correct product and category`() = runTest {
    val matcher = matcher(
  ...
  @Test
  fun `case insensitive match still finds product`() = runTest {
    val matcher = matcher(
      categoryWithItems(dairyCategory, milkProduct),
    )
    val proposal = GroceryProposal(
  ...
  @Test
  fun `fuzzy match on product name with typo still returns Match`() = runTest {
    val matcher = matcher(
      categoryWithItems(dairyCategory, milkProduct),
    )
    val proposal = GroceryProposal(
  ...
  @Test
  fun `no match when database is empty`() = runTest {
    val matcher = matcher()
    val proposal = GroceryProposal(
      productName = "Milk",
      quantity = 1,
  ...
  @Test
  fun `no match for completely unrelated product`() = runTest {
    val matcher = matcher(
      categoryWithItems(dairyCategory, milkProduct),
    )
    val proposal = GroceryProposal(
  ...
  @Test
  fun `picks best candidate across multiple categories`() = runTest {
    val matcher = matcher(
  ...
  @Test
  fun `category name contributes to score — wrong category lowers match`() = runTest {
    // Same product name exists in both categories; proposal category points to Dairy
    val meatMilk = Product(
      id = "prod-4",
      categoryId = "cat-2",
      name = "Milk",
      createdAt = Instant.fromEpochSeconds(0),
